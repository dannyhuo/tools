...
public class Encript extends JPanel implements ActionListener{
	
	/**
	 * 
	 */
	private static final long serialVersionUID = -8910413119713693650L;
	//行 --01
	...
	private JComboBox<String> ecrTimes = null;
	...
	public void initMyPanel(){
		...
		this.ecrTimes = new JComboBox<String>(petStrings);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == this.btnPreview){
			...
			if(!"".equals(path.trim())){
				String toName = "";
				if("".equals(this.toPath.getText().trim())){
					String folder = path.substring(0,path.lastIndexOf("/"))+"/ecr"+System.currentTimeMillis();
					FileUtil.mkDir(folder);
					toName = folder+"/"+path.substring(path.lastIndexOf("/"));
				}else{
					toName = this.toPath.getText() + "/" + path.substring(path.lastIndexOf("/"));
				}
				FileUtil.ecrpt(path, toName);
			}
